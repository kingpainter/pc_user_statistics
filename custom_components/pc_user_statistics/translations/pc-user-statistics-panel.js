...
class PcUserStatisticsPanel extends HTMLElement {
  ...
  async _loadForTab() {
    ...
    try {
      ...
      if (tab === "statistics" || tab === "users") {
        ...
        // System info changes rarely — still worth refreshing
      ...
      // history + config: never auto-refresh — data is days/session-scoped
      this._errCount = 0;
    } catch (e) {
      this._errCount++;
  ...
  _headerHTML() {
    // Defensive: ensure current_user is a plain string, not an object
  ...
  _statisticsHTML() {
    ...
    s = {...s, current_user: cu};  // normalise for all downstream uses
  ...
  _css() {
    ...
    * { box-sizing: border-box; }
    ...
    .header { display:flex; align-items:center; justify-content:space-between;
      background:#3d4a2e; color:white; padding:16px 20px; border-radius:16px 16px 0 0; }
